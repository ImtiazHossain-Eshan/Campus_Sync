document.addEventListener('DOMContentLoaded', () => {
  ...
  searchInput.addEventListener('input', async (e) => {
    ...
    try {
      ...
      users.forEach((user, index) => {
        ...
        card.innerHTML = `
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 break-all max-w-[180px]">${user.email}</p>
              ${statusLabel}
            </div>
          </div>
          ${buttonHTML}
        ...
        setTimeout(() => card.classList.remove('opacity-0', 'translate-y-2'), index * 60);
      });
  ...
  window.sendRequest = async function(friendId) {
  ...
  async function loadFriends() {
    ...
    data.accepted.forEach(friend => {
      ...
      const card = document.createElement('div');
      card.className = `bg-white p-4 rounded-xl border shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col h-full`;
      card.innerHTML = `
      ...
      card.innerHTML = `
        <div class="flex items-center gap-4">
          ...
          <div class="min-w-0">
            ...
            <p class="text-sm text-gray-500 break-all max-w-[180px]">${req.email ?? ''}</p>
          </div>
        </div>
