document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      users.forEach((user, index) => {
        ...
        if (!isFriend && !isPending) {
          ...
          button.addEventListener('click', () => sendRequest(user.id));
  ...
  async function loadFriends() {
    ...
    data.accepted.forEach(friend => {
      ...
      card.innerHTML = `
        <div class="flex items-center gap-4 mb-4">
          <img src="${profilePic}" class="w-16 h-16 rounded-full object-cover border-2 border-blue-500" alt="Friend">
          <div>
            <p class="font-semibold text-gray-800">${friend.name}</p>
            <p class="text-sm text-gray-500 break-all max-w-[180px]">${friend.email ?? ''}</p>

          </div>
        </div>`;

      const actions = document.createElement('div');
      actions.className = 'flex flex-wrap gap-3 mt-auto';

      const viewBtn = document.createElement('a');
      viewBtn.href = `view_friend.php?friend_id=${friend.id}`;
      viewBtn.className = 'bg-indigo-600 text-white text-sm px-4 py-1.5 rounded-full hover:bg-indigo-700 transition';
      viewBtn.textContent = 'View';

      const removeBtn = document.createElement('button');
      removeBtn.className = 'bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 px-4 py-1.5 rounded-full text-sm font-medium transition';
      removeBtn.textContent = 'Remove';
      removeBtn.addEventListener('click', () => confirmRemove(friend.id, friend.name));
    ...
    data.pending.forEach(req => {
      ...
      acceptBtn.className = 'bg-green-500 text-white px-4 py-1.5 rounded-full hover:bg-green-600 transition text-sm font-medium';
      acceptBtn.textContent = 'Accept';
      acceptBtn.addEventListener('click', () => acceptRequest(req.id));
    ...
    data.sent.forEach(req => {
      ...
      cancelBtn.className = 'bg-red-100 text-red-600 hover:bg-red-200 border border-red-300 px-4 py-1.5 rounded-full text-sm font-medium transition';
